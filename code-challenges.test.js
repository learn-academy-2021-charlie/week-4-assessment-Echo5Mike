...
//create a test and let it fail
//function
    // will take in testArray1,2
    // concat two arrays then sort?
    // we sort then concat
//duplicates   
    //maybe a for loop simulir to febanchi 
    //.filiter
    // going to need any empty array to hold new numbers


describe("function that test two arrays", () => {
    test("takes in two arrays of numbers, returns ONE array with no duplicates", () => {
      expect(twins([3, 7, 10, 5, 4, 3, 3],[7, 8, 2, 3, 1, 5, 4])).toContain(3, 7, 10, 5, 4, 8, 2, 1)
    })
})
const twins = (array1, array2)=>{
    newArray = (array2.concat(array1)).sort((a,b) => a-b)
    noDubs = []
    for(let i=0; i<newArray.length; i++){
        if(newArray[i] !== newArray[i+1])// I really don't know how I did it but I did
        noDubs.push(newArray[i])
    }
    return noDubs//there are no duplictes in the array of numbers but they are sorted from lwo to high
}
